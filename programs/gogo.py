...
class GoGoApi:
    ...
    def search(self, query, url_only=False):
        soup = bs(
            requests.get(f"https://{self.host}/search.html?keyword=" + query).content,
            "html.parser",
        )
        ...
        if url_only:
            ...
            for i in animes:
                ...
                results.append(Anime(url, img, released, title, None))
            return results
    ...
    def home(self):
        soup = bs(requests.get(f"https://{self.host}").content, "html.parser")
        div = soup.find("ul", "items")
        animes = div.find_all("li")
    ...
    def get_links(self, url):
        ...
        soup = bs(requests.get(f"https://{self.host}/{url}").content, "html.parser")
        ...
        if "dub" in anime:
            ...
            data["DL"]["SUB"] = dlink
            anime = (
                anime.split("-episode-")[0]
                + "-dub"
                + "-episode-"
                + anime.split("-episode-")[1]
            )
            soup = bs(
                requests.get(f"https://{self.host}/{anime}").content, "html.parser"
            )
